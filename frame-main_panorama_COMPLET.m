%-----------------------------Vidéo panoramas-----------------------------%
%--Réaliser des images panoramiques en prenant comme paramètre une vidéo--%

%%Etape 1 : Charger la vidéo et écire les différentes trames

obj = VideoReader('foret_aerienne_video.mp4');
numFrames = obj.NumberOfFrames;
 for k = 1:20:numFrames
     frame = read(obj,k);
      imshow(frame);
      imwrite(frame,strcat('D:\Polytech\APP\Prog_imagerie\Images_test',num2str(k),'.jpg'),'jpg');
 end

%Charger et afficher les différenetes images qu'il faut transformer
tic
multiple_images = imageSet('D:\Polytech\APP\Prog_imagerie\Images_test');
montage(multiple_images.ImageLocation);

%%Etape 2: Les images panoramiques:

% Read the first image from the image set.
I = read(multiple_images, 1);

% Trouver les points d'int?r?ts de la premi?re image
grayImage = rgb2gray(I);
points = detectSURFFeatures(grayImage);
[features, points] = extractFeatures(grayImage, points);

% Initialize all the transforms to the identity matrix.
tforms(multiple_images.Count) = projective2d(eye(3));

% Iterate over remaining image pairs
for n = 2:multiple_images.Count
...
% Create the panorama.
for i = 1:multiple_images.Count
    
    I = read(multiple_images, i);   
